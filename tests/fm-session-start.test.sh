...
# Markers today: CLAUDECODE (claude), CODEX_SHELL (the maintained fork's Codex
# launcher), PI_CODING_AGENT plus FM_PI_HARNESS (Pi family), GROK_AGENT (grok).
# Opencode remains ancestry-only. Without this, a local harness can leak into
# cases that pin a different fake process tree while CI still passes.
run_session_start() {
  local home=$1 root=$2 path=$3 pi_harness=${4:-}
  if [ -n "$pi_harness" ]; then
    env -u CLAUDECODE -u CODEX_SHELL -u GROK_AGENT PI_CODING_AGENT=true FM_PI_HARNESS="$pi_harness" \
      FM_HOME="$home" FM_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  else
    env -u CLAUDECODE -u CODEX_SHELL -u PI_CODING_AGENT -u FM_PI_HARNESS -u GROK_AGENT \
      FM_HOME="$home" FM_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  fi
}
