...
make_fake_root() {
  ...
  # fm-session-lock-lib.sh: the other sibling bin/backends/tmux.sh sources
  # unconditionally. Absent, loading the adapter aborts the teardown mid-cleanup
  # - silently, because the kill call it aborts in redirects stderr - so the
  # tasktmp root is never removed.
...
test_teardown_skips_gracefully_without_tasktmp() {
  ...
  ln -s "$ROOT/bin/fm-composer-lib.sh" "$fake/bin/fm-composer-lib.sh"
  ln -s "$ROOT/bin/fm-nm-run-lib.sh" "$fake/bin/fm-nm-run-lib.sh"
  # fm-session-lock-lib.sh: sourced unconditionally by bin/backends/tmux.sh.
  ln -s "$ROOT/bin/fm-session-lock-lib.sh" "$fake/bin/fm-session-lock-lib.sh"
  ln -s "$ROOT/bin/fm-lock-lib.sh" "$fake/bin/fm-lock-lib.sh"
  # fm-gate-refuse-lib.sh: teardown sources it before any fleet mutation.
  ln -s "$ROOT/bin/fm-gate-refuse-lib.sh" "$fake/bin/fm-gate-refuse-lib.sh"
