...
# Briefs generated by fm-brief carry the review-evidence marker. They cannot
# launch until the captain and firstmate have made the proof of completion
# explicit in the adjacent structured contract.
if [ "$KIND" != secondmate ] && grep -Fq '# Review evidence' "$BRIEF"; then
  EVIDENCE_CONTRACT="$DATA/$ID/evidence-contract.json"
  if ! [ -f "$EVIDENCE_CONTRACT" ] || ! jq -e '
    .schemaVersion == 1 and
    .status == "agreed" and
    (.acceptance | type == "array" and length > 0) and
    (.automatedChecks | type == "array") and
    (.limitations | type == "array") and
    (.visualEvidence | type == "object") and
    (.visualEvidence.requirement == "required" or
     .visualEvidence.requirement == "optional" or
     .visualEvidence.requirement == "not-applicable") and
    (.visualEvidence.targets | type == "array") and
    (.visualEvidence.requirement != "required" or (.visualEvidence.targets | length > 0))
  ' "$EVIDENCE_CONTRACT" >/dev/null 2>&1; then
    echo "error: evidence contract is not agreed for $ID" >&2
    echo "set acceptance criteria, checks, visual requirement, targets, and limitations in $EVIDENCE_CONTRACT; then set status to agreed" >&2
    exit 1
  fi
fi
