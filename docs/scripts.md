...
| `fm-session-lock-lib.sh` | Shared session-lock ownership identity (harness ancestry walk, holder liveness, and Codex session-token/launcher-pid ownership) for fm-lock.sh and the Claude Stop auto-arm |
...
| `fm-scotty-liaison.sh`   | FirstMate-owned Cycle fan-out: consume durable Scotty notifications, validate child bindings, and delegate through `fm-spawn.sh` |
