...
A numeric session-lock owner that fails the shared `fm_harness_pid_alive` predicate is offered to `bin/fm-lock.sh` for a guarded claim before auto-arm state changes, while a live owner, absent lock, or malformed lock keeps the competing hook inert.
`bin/fm-lock.sh` remains the single acquisition owner and refuses that claim, so a lock a live Codex launcher still holds through its session token is never reclaimed here even though the launcher pid is not itself a verified harness.
